IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'DIAMONDPROJECT')
BEGIN
    CREATE DATABASE DIAMONDPROJECT;
END
GO

USE DIAMONDPROJECT
GO

DROP TABLE IF EXISTS DIAMONDCERTIFICATE;
DROP TABLE IF EXISTS SHIPPINGADDRESS;
DROP TABLE IF EXISTS PAYMENT;
DROP TABLE IF EXISTS WARRANTY;
DROP TABLE IF EXISTS ORDERITEM;
DROP TABLE IF EXISTS ORDERS;
DROP TABLE IF EXISTS CUSTOMER;
DROP TABLE IF EXISTS REFUNDPRODUCT;
DROP TABLE IF EXISTS PENDANTPAIRING;
DROP TABLE IF EXISTS EARRINGPAIRING;
DROP TABLE IF EXISTS RINGPAIRING;
DROP TABLE IF EXISTS PENDANT;
DROP TABLE IF EXISTS EARRING;
DROP TABLE IF EXISTS RING;
DROP TABLE IF EXISTS DIAMOND;

CREATE TABLE CUSTOMER (
  CustomerID UNIQUEIDENTIFIER PRIMARY KEY,
  FirstName VARCHAR(50),
  LastName VARCHAR(50),
  Email VARCHAR(100),
  Password VARCHAR(100),
  PhoneNumber VARCHAR(20),
  RegistrationDate DATETIME
);

CREATE TABLE ORDERS (
  OrderID UNIQUEIDENTIFIER PRIMARY KEY,
  CustomerID UNIQUEIDENTIFIER,
  OrderDate DATETIME,
  TotalAmount MONEY,
  OrderStatus VARCHAR(50),
  FOREIGN KEY (CustomerID) REFERENCES CUSTOMER(CustomerID)
);

CREATE TABLE ORDERITEM (
  OrderItemID UNIQUEIDENTIFIER PRIMARY KEY,
  OrderID UNIQUEIDENTIFIER,
  ProductID UNIQUEIDENTIFIER,
  Price MONEY,
  ProductType VARCHAR(50),
  FOREIGN KEY (OrderID) REFERENCES ORDERS(OrderID)
);

CREATE TABLE SHIPPINGADDRESS (
  AddressID UNIQUEIDENTIFIER PRIMARY KEY,
  CustomerID UNIQUEIDENTIFIER,
  AddressLine1 VARCHAR(100),
  AddressLine2 VARCHAR(100),
  City VARCHAR(50),
  District VARCHAR(50),
  FOREIGN KEY (CustomerID) REFERENCES CUSTOMER(CustomerID)
);

CREATE TABLE PAYMENT (
  PaymentID UNIQUEIDENTIFIER PRIMARY KEY,
  OrderID UNIQUEIDENTIFIER UNIQUE,
  PaymentMethod VARCHAR(50),
  PaymentDate DATETIME,
  PaymentStatus VARCHAR(50),
  PaymentMessage VARCHAR(200),
  Amount MONEY,
  FOREIGN KEY (OrderID) REFERENCES ORDERS(OrderID)
);

CREATE TABLE WARRANTY (
  WarrantyID UNIQUEIDENTIFIER PRIMARY KEY,
  OrderItemID UNIQUEIDENTIFIER UNIQUE,
  WarrantyStartDate DATETIME,
  WarrantyEndDate DATETIME,
  FOREIGN KEY (OrderItemID) REFERENCES ORDERITEM(OrderItemID)
);

CREATE TABLE DIAMOND (
  D_ProductID UNIQUEIDENTIFIER PRIMARY KEY,
  Name VARCHAR(50),
  Price MONEY,
  ImageURL VARCHAR(255),
  D_Type VARCHAR(50),
  CaratWeight VARCHAR(20),
  Color VARCHAR(20),
  Clarity VARCHAR(20),
  Cut VARCHAR(20)
);

CREATE TABLE RING (
  RingID UNIQUEIDENTIFIER PRIMARY KEY,
  Name VARCHAR(50),
  Price MONEY,
  StockQuantity INT,
  ImageURL VARCHAR(255),
  MetalType VARCHAR(50),
  RingSize VARCHAR(20)
);

CREATE TABLE EARRING (
  EarringID UNIQUEIDENTIFIER PRIMARY KEY,
  Name VARCHAR(50),
  Price MONEY,
  StockQuantity INT,
  ImageURL VARCHAR(255),
  MetalType VARCHAR(50)
);

CREATE TABLE PENDANT (
  PendantID UNIQUEIDENTIFIER PRIMARY KEY,
  Name VARCHAR(50),
  Price MONEY,
  StockQuantity INT,
  ImageURL VARCHAR(255),
  MetalType VARCHAR(50)
);

CREATE TABLE RINGPAIRING (
  R_ProductID UNIQUEIDENTIFIER PRIMARY KEY,
  RingID UNIQUEIDENTIFIER,
  DiamondID UNIQUEIDENTIFIER,
  FOREIGN KEY (RingID) REFERENCES RING(RingID),
  FOREIGN KEY (DiamondID) REFERENCES DIAMOND(D_ProductID)
);

CREATE TABLE EARRINGPAIRING (
  E_ProductID UNIQUEIDENTIFIER PRIMARY KEY,
  EarringID UNIQUEIDENTIFIER,
  DiamondID UNIQUEIDENTIFIER,
  FOREIGN KEY (EarringID) REFERENCES EARRING(EarringID),
  FOREIGN KEY (DiamondID) REFERENCES DIAMOND(D_ProductID)
);

CREATE TABLE PENDANTPAIRING (
  P_ProductID UNIQUEIDENTIFIER PRIMARY KEY,
  PendantID UNIQUEIDENTIFIER,
  DiamondID UNIQUEIDENTIFIER,
  FOREIGN KEY (PendantID) REFERENCES PENDANT(PendantID),
  FOREIGN KEY (DiamondID) REFERENCES DIAMOND(D_ProductID)
);

CREATE TABLE REFUNDPRODUCT (
  RF_ProductID UNIQUEIDENTIFIER PRIMARY KEY,
  Name VARCHAR(50),
  Price MONEY,
  ImageURL VARCHAR(255),
  Specifications TEXT,
  CaratWeight VARCHAR(20),
  Color VARCHAR(20),
  Clarity VARCHAR(20),
  Cut VARCHAR(20)
);

CREATE TABLE DIAMONDCERTIFICATE (
  CertificateID UNIQUEIDENTIFIER PRIMARY KEY,
  ProductID UNIQUEIDENTIFIER,
  CertificateNumber VARCHAR(50),
  IssuedBy VARCHAR(50),
  IssueDate DATETIME
);

ALTER TABLE ORDERITEM
  ADD CONSTRAINT FK_Orderitem_Diamond FOREIGN KEY (ProductID) REFERENCES DIAMOND(D_ProductID);

ALTER TABLE ORDERITEM
  ADD CONSTRAINT FK_Orderitem_Ringpairing FOREIGN KEY (ProductID) REFERENCES RINGPAIRING(R_ProductID);

ALTER TABLE ORDERITEM
  ADD CONSTRAINT FK_Orderitem_Earringpairing FOREIGN KEY (ProductID) REFERENCES EARRINGPAIRING(E_ProductID);

ALTER TABLE ORDERITEM
  ADD CONSTRAINT FK_Orderitem_Pendantpairing FOREIGN KEY (ProductID) REFERENCES PENDANTPAIRING(P_ProductID);

ALTER TABLE ORDERITEM
  ADD CONSTRAINT FK_Orderitem_Refundproduct FOREIGN KEY (ProductID) REFERENCES REFUNDPRODUCT(RF_ProductID);

ALTER TABLE DIAMONDCERTIFICATE
  ADD CONSTRAINT FK_DiamondCertificate_Diamond FOREIGN KEY (ProductID) REFERENCES DIAMOND(D_ProductID);

ALTER TABLE DIAMONDCERTIFICATE
  ADD CONSTRAINT FK_DiamondCertificate_Refundproduct FOREIGN KEY (ProductID) REFERENCES REFUNDPRODUCT(RF_ProductID);
