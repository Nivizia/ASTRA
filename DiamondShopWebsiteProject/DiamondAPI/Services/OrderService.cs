    ...
    public class OrderService
    {
        ...
        public async Task<bool> SendOrderConfirmRequest()
        {
            ...
            foreach (var order in orders)
            {
                ...
                {
                    string confirmationUrl = $"http://localhost:5173/confirmation-email?c={order.CustomerId}&o={order.OrderId}";
                    string emailBody = $@"<p>Your order with ID: {order.OrderId} has been received, please press 
                <a href='{confirmationUrl}'>this link</a> to confirm your order.</p>";
