...
const OrderHistory = () => {
  ...
  if (!loading && (orders.length === 0 || !orders.some(order => ["Payment Received", "Processing", "Confirmation Sent", "Postponed", "Cancelled", "Confirmed", "Completed"].includes(order.orderStatus)))) {
  ...
  return (
    <div className={styles.orderHistoryContainer}>
      ...
      {loading ? <CircularIndeterminate size={56} /> : (
        orders.filter(order => ["Payment Received", "Processing", "Confirmation Sent", "Postponed", "Cancelled", "Confirmed", "Completed"].includes(order.orderStatus))
          .map((order) => (
